...
def dijkstra_path(graph, start_server):
    return nx.single_source_dijkstra_path_length(graph, start_server)


start_server = "Server11_backup"

shortest_path = dijkstra_path(G, start_server)

# Виводимо найкоротші шляхи
print(f"Shortest path to  {start_server}:")
for server, distance in shortest_path.items():
    print(f"From {start_server} to {server}: {distance} edge")
